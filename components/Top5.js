...
const Card = ({CollectionName,amountSum, collectionImage, router }) => {
    ...
    >
      <span className="font-extrabold capitalize text-xl  ">{`${(Math.round(amountSum * 100) / 100).toFixed(2)} $${coinName(router.query.chain)}`}</span>
        ...
        <div className="rounded-2xl my-5 p-2  text-[#85848b]">
            ...
            <div className="flex flex-col md:flex-row gap-2 justify-between mb-2 pb-1 ">
                ...
                <div className="basis-auto flex gap-2 mb-1 ">
                    <input type="button" onClick={()=>setQuery(1)} className={`cursor-pointer ${query===1 && "text-white"} p-2 px-3 rounded-lg hover:bg-black/25`} value="1 Day" />
                    <input type="button" onClick={()=>setQuery(7)} className={`cursor-pointer ${query===7 && "text-white"} p-2 px-3 rounded-lg hover:bg-black/25`} value="7 Days" />
                    <input type="button" onClick={()=>setQuery(30)} className={`cursor-pointer ${query===30 && "text-white"} p-2 px-3 rounded-lg hover:bg-black/25`} value="30 Days" />
            ...
            <div className={`px-3 grid gap-4 grid-cols-2 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5  `}>
                {
                    list.map((sale,i) => (
